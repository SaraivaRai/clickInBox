CREATE TABLE boxes (
  ...
  musica TEXT,
  apresentacao_tipo VARCHAR(10) NOT NULL DEFAULT 'texto'
    CHECK (apresentacao_tipo IN ('texto', 'imagem')),
  apresentacao_imagem TEXT,
  imagem_principal_original TEXT,
  imagem_foco_x NUMERIC(5,2)
    CHECK (imagem_foco_x IS NULL OR imagem_foco_x BETWEEN 0 AND 100),
  imagem_foco_y NUMERIC(5,2)
...
CREATE TABLE publicacoes_protagonista (
  id SERIAL PRIMARY KEY,
  box_id INTEGER NOT NULL REFERENCES boxes(id),
  usuario_id INTEGER NOT NULL REFERENCES usuarios(id),
